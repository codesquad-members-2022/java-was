...
- 로그인을 확인하는 로직이 미흡하다. 
  ...
  - `logged_in` 쿠키의 값을 읽어서 로그인 여부를 확인하는데, 서버를 재시작했을 때 서버의 세션 정보는 초기화되고 클라이언트의 쿠키는 그대로 남아있어서 로그인 체크는 통과했지만 현재 로그인 중인 유저의 정보를 읽어올 수 없어서 오류가 발생한다. GitHub 등의 사이트에서도 true/false, yes/no 형식의 로그인 쿠키를 사용하는데, 로그인 쿠키의 용도가 정확히 무엇인지, 매번 세션을 읽어서 로그인 여부를 검사하면 성능 저하가 발생하는지 알아보아야겠다.
